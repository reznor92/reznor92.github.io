...
(function (global, factory) {
    ...
    niceDatePicker.prototype.buildUi = function (year, month) {
        ...
        this.dayWords = [['一', '二', '三', '四', '五', '六', '日'], ['пн', 'вт', 'ср', 'чт', 'пт', 'сб', 'вс']];
        this.enMonthsWords = ['Январь', 'Февраль', 'Март', 'Апрель', 'Май', 'Июнь', 'Июль', 'Август', 'Сентябрь', 'Октябрь', 'Ноябрь', 'Декабрь'];
        ...
        if (!this.$params.mode) {
            ...
            html += '<span class="nice-date-title">' + this.enMonthsWords[this.monthData.month - 1] + '</span>';
        ...
        for (var i = 0; i < this.monthData.date.length; i++) {
            ...
            html += '<td class="' + this.monthData.date[i].styleCls + '" data-date="' + this.enMonthsWords[this.monthData.month - 1] + '/' + this.monthData.date[i].showDate + '">' + this.monthData.date[i].showDate + '</td>';
            if (i % 7 === 6) {
                html += '</tr>';
            }
        }
